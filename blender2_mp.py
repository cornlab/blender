...
def parse_arguments():
    ...
    parser.add_argument('-j', '--nproc', type=int, default=2, help='number of processes to use (default 2)')
    ...
    for site, n_ends in both_starts.items():
        ...
        if pam_minus in nuc.pam_seqs:
            ...
            if result['success']:
                ...
                if args.verbose:
                    log.info(sitedict)
                allsites['sites'].append(sitedict)
            else:
                if args.verbose:
        ...
        if pam_plus in nuc.pam_seqs:
            ...
            if result['success']:
                ...
                if args.verbose:
                    ...
                    if result['s'] < 0 or result['e'] < 0:
                        log.info(f"{chromosome}:{s1}-{e1}\t{site1}\t{score}\t{n_ends}\t{strand}\t{pam_fullseq}\t{guide} FILTERED: too close to start of chromosome")
    allsites['bg_discoscores'] = bg_discoscores
    return allsites
...
if __name__ == '__main__':
    ...
    with mp.Pool(args.nproc) as pool:
        for chromosome in edited_bamfile.references:
            result = pool.apply_async(check_chromosome, (chromosome,))
            results.append(result)
        pool.close()
        pool.join()
    log.info(f"Found {len([x for x in results if x is not None])} unfiltered sites")
    for result in results:
        if result.get() is None:
            continue
        chr_discoscores = result.get()['bg_discoscores']
        chromosome_sites = result.get()['sites']
        if len(chr_discoscores) > 0:
            bg_discoscores.extend(chr_discoscores)
        if len(chromosome_sites) > 0:
            for sitedict in chromosome_sites:
